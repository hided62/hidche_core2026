...
const installImages = async (page: Page) => {
    await page.route('**/image/**', async (route) => {
        ...
        const candidates = imageRoots.flatMap((root) => [
            resolve(root, relative),
            resolve(root, 'game', relative),
        ]);
...
const installMainFixture = async (page: Page, failRecords = false) => {
    ...
    await page.route('**/che/api/trpc/**', async (route) => {
        const results = operationNames(route).map((operation) => {
            ...
            if (operation === 'general.getFrontRecords') {
                return failRecords ? errorResponse(operation, '동향 정보를 불러오지 못했습니다.') : response(frontRecords);
            }
...
test('shows the current in-game map and all three recent record streams', async ({ page }) => {
    ...
    await expect(page.locator('.recent-log-line span').first()).toHaveCSS('color', 'rgb(255, 255, 0)');
